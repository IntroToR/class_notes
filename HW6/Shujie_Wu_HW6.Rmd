...
## 5
```{r}
# Creating the table with arrival delay average versus months
mean_arr_delay <- select(flights_airlines, arr_delay, month) %>%
  na.omit()%>%
  group_by(month)%>% 
  summarize(mean = mean(arr_delay))
# Visualizing the table
ggplot(mean_arr_delay, aes(x = month, y = mean))+
    geom_bar(stat = "identity")+
    ggtitle("Average Arrival Delay by Month")+
    xlab("months")+ylab("average arrival delay")+
