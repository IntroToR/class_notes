...
#Coloring observations according to h#
h1_meas <- refpheno_long %>%
  filter(hnumber == "h1")

h2_meas <- refpheno_long %>%
  filter(hnumber == "h2")

h3_meas <- refpheno_long %>%
  filter(hnumber == "h3")

h4_meas <- refpheno_long %>%
  filter(hnumber == "h4")
```
...
ggplot(long_pheno, aes(x=as.factor(Range_and_Row_Y), y=Y))+
  ...
  facet_wrap(~Range_and_Row, nrow=2, ncol=1)
